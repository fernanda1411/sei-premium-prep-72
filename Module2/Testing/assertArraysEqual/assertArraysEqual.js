...
// console output:
// FAILED [generates first 7 Fibonacci numbers] Expected "1, 1, 2, 3

*/

function assertArraysEqual(actual, expected, testName) {
  //actual - array of scalar values
  //expected - array of scalar values
  //testName - string

  //creat a variable to check if they(actual, expected) have the same length
  let areEqualLength = actual.length === expected.length;

  //declare areEqualItems to be true
  let areEqualItems = true;

  //iterate over the input
  //enquanto o i for menor do que expected.length ele vai continuar a rodando
  for (let i = 0; i < expected.length; i++) {
    //se o tamanho do input da hora for diferente do atual[i]
  ...
  //se areEqualLenght e areEqualItems forem iguais
